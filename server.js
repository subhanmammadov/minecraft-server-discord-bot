...
client.commands = new Discord.Collection();//techno
client.aliases = new Discord.Collection();//techno
fs.readdir('./techno/', (err, files) => {//techno
  if (err) console.error(err);//techno
  log(`${files.length} komut yüklenecek.`);//techno
  files.forEach(f => {//techno
    let props = require(`./techno/${f}`);//techno
    log(`Yüklenen komut: ${props.help.name}.`);//techno
    client.commands.set(props.help.name, props);//techno
    props.conf.aliases.forEach(alias => {//techno
      client.aliases.set(alias, props.help.name);//techno
    });//techno
  });//techno
});//techno
//techno
client.reload = command => {
  return new Promise((resolve, reject) => {
    try {
      delete require.cache[require.resolve(`./techno/${command}`)];
      let cmd = require(`./techno/${command}`);
